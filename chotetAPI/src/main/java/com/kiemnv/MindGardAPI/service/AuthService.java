...
public class AuthService {
    ...
    @Value("${oauth2.google.client-ids:}")
    ...
    public AuthResponse googleLogin(String idTokenString, HttpServletResponse response) {
        try {
            GoogleIdTokenVerifier verifier = new GoogleIdTokenVerifier.Builder(
                    ...
                    .setAudience(googleClientIds != null ? googleClientIds : Collections.emptyList())
                    .build();
    ...
    public AuthResponse refreshToken(String oldRefreshToken, HttpServletResponse response) {
        ...
        if (!storedToken.getIpAddress().equals(currentIpAddress)) {
            log.warn("Refresh token used from different IP for user: {}. Original IP: {}, Current IP: {}. Allowing for mobile compatibility.",
                    user.getUsername(), storedToken.getIpAddress(), currentIpAddress);
            // Optionally update the IP address to the new one
            storedToken.setIpAddress(currentIpAddress);
